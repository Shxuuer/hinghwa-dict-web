<template>
  <div class="login">
    ...
    <a-row :gutter="[0,20]" align="middle" justify="start" type="flex">
      ...
      <a-col span="8">
        <a-input v-model:value="username" placeholder="输入用户名"/>
      </a-col>
      <a-col span="8"/>
      ...
      <a-col span="8">
        <a-input-password v-model:value="password" placeholder="输入密码" size="middle"/>
      </a-col>
    ...
    login () {
      axios.post('/login', {
        ...
        if (!this.$store.getters.user.county) {
          message.warning('为了便于后续使用，请设置自己的所在乡镇~')
        }
